...
import { SOLUTIONS } from "../CommonComp/Solutions"


import './styles.css'
import Keyboard from "../CommonComp/Keyboard";
...
function DynamicGrid () {
    ...
    const [values, setValues] = useState<string [][]>(Array.from(Array(6), () => Array(5).fill("")))
    ...
    const getTodaySolution  = () => {
        ...
        const currDateString = date?.substring(0,2) + "/" +  date?.substring(2,4) + "/" +  date?.substring(4)
        const currDate = new Date(currDateString);
        const JUN_18TH : Date = new Date("06/18/2022")

        currDate.setHours(0, 0, 0, 0);


        const difference = currDate.getTime() - JUN_18TH.getTime();
        const dayDifference = difference / (1000 * 3600 * 24);
        setSolution(SOLUTIONS[dayDifference])
    ...
    const writeGuessToDatabase = () => {
        ...
        const updates : {[key: string] : {[key: string] : Guess}} = {};

        console.log(postData)

        updates['/users/' + user?.uid + '/guesses/' + params.date] = postData;
